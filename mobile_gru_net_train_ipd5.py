...
from cp_mobile_dataset import DOADataset_15_ipd
import wandb
import json
from cp_mobile_gru import CPGruNet5
import argparse


class DOA_MUSIC_lhw(pl.LightningModule):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.model = CPGruNet5(
            n_classes=config.n_classes,
            in_channels=config.in_channels,
        )

        self.criterion = nn.CrossEntropyLoss()
        self.train_acc = Accuracy(task="multiclass", num_classes=self.config.n_classes)
        self.val_acc = Accuracy(task="multiclass", num_classes=self.config.n_classes)
        self.test_acc = Accuracy(task="multiclass", num_classes=self.config.n_classes)
        self.lr = self.config.lr
        self.weight_decay = self.config.weight_decay
        self.test_loss = 0.0
        self.test_acc_value = 0.0

    def forward(self, stft_cat):
        return self.model(stft_cat)

    def training_step(self, batch, batch_idx):
        stft_cat, label = batch
        logits = self(stft_cat)
    ...
    def test_step(self, batch, batch_idx):
        ...
        logits = self(stft_cat)
        loss = self.criterion(logits, label)
        acc = self.test_acc(logits, label)
        self.test_loss = loss.item()
        self.test_acc_value = acc.item()
        self.log("test/loss", loss, on_epoch=True, prog_bar=True)
        self.log("test/acc", acc, on_epoch=True, prog_bar=True)

    def on_test_epoch_end(self):
        self.test_loss = self.trainer.callback_metrics["test/loss"].item()
        self.test_acc_value = self.trainer.callback_metrics["test/acc"].item()
...
def train(config):
    ...
    train_set = DOADataset_15_ipd(
         jsonl_path="/home/lihaowen/BEWO/BEWO_SS_Annotation_v1/BEWO_SS_Annotation_v1/audiocaps_single_train.jsonl",
         audio_root="/home/lihaowen/BEWO/BEWO_SS_Audio_v1/BEWO_SS_Audio_v1/audiocaps_single_train")
    val_set = DOADataset_15_ipd(
         jsonl_path="/home/lihaowen/BEWO/BEWO_SS_Annotation_v1/BEWO_SS_Annotation_v1/audiocaps_single_val.jsonl",
         audio_root="/home/lihaowen/BEWO/BEWO_SS_Audio_v1/BEWO_SS_Audio_v1/audiocaps_single_val")
    test_set = DOADataset_15_ipd(
    ...
    '''
    train_set = DOADataset_15_stft(
        jsonl_path="F:/BEWO\BEWO-1M-SS/BEWO_SS_Annotation_v1/BEWO_SS_Annotation_v1/audiocaps_single_train.jsonl",
        audio_root="F:/BEWO/BEWO-1M-SS/BEWO_SS_Audio_v1/BEWO_SS_Audio_v1/audiocaps_single_train")
    val_set = DOADataset_15_stft(
        jsonl_path="F:/BEWO\BEWO-1M-SS/BEWO_SS_Annotation_v1/BEWO_SS_Annotation_v1/audiocaps_single_val.jsonl",
        audio_root="F:/BEWO/BEWO-1M-SS/BEWO_SS_Audio_v1/BEWO_SS_Audio_v1/audiocaps_single_val")
    test_set = DOADataset_15_stft(
        jsonl_path="F:/BEWO\BEWO-1M-SS/BEWO_SS_Annotation_v1/BEWO_SS_Annotation_v1/audiocaps_single_test.jsonl",
        audio_root="F:/BEWO/BEWO-1M-SS/BEWO_SS_Audio_v1/BEWO_SS_Audio_v1/audiocaps_single_test")
    '''
    train_dl = DataLoader(
        dataset=train_set,
    ...
    # ==== 初始化模型 ====
    pl_module = DOA_MUSIC_lhw(config)
    ...
    pl_module.logger.experiment.log(
        {"test/best_ckpt_path": checkpoint_callback.best_model_path, "test_best/acc": pl_module.test_acc_value,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--experiment_name', type=str, default="mobile_gru_net5_15_besttest_right13_ipd_bs256_pat20")
